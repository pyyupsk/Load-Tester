...
async def load_test(url, num_requests, concurrency, proxies, headers, method, data):
    connector = aiohttp.TCPConnector(ssl=ssl.create_default_context(), limit=concurrency)

    async with aiohttp.ClientSession(connector=connector, trust_env=True) as session:
        semaphore = asyncio.Semaphore(concurrency)
        progress_bar = tqdm(total=num_requests, desc="Requests", unit="req", bar_format="{l_bar}%s{bar}%s{r_bar}" % (Fore.GREEN, Fore.RESET))
    ...
    print(f"\n{Fore.GREEN}Successful Requests:{Style.RESET_ALL} {len(successful_requests)} ({len(successful_requests)/num_requests*100:.2f}%)")
    print(f"{Fore.RED}Failed Requests:{Style.RESET_ALL} {len(failed_requests)} ({len(failed_requests)/num_requests*100:.2f}%)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"{Fore.CYAN}Powerful Load Testing Tool{Style.RESET_ALL}")
    ...
    with ThreadPoolExecutor() as executor:
        future = executor.submit(
            asyncio.run,
            run_load_test(args.url, args.num_requests, args.concurrency, args.proxy_file, headers, args.method, args.data)
        )
