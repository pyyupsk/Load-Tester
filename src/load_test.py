...
async def load_test(url, num_requests, concurrency, proxies, headers, method, data):
    ...
    async def bounded_send_request():
    ...
    tasks = [bounded_send_request() for _ in range(num_requests)]
    results = await asyncio.gather(*tasks, return_exceptions=True)

    progress_bar.close()

    # Filter successful and failed requests
    successful_requests = [r for r in results if isinstance(r, tuple) and isinstance(r[0], int)]
    failed_requests = [r for r in results if isinstance(r, tuple) and isinstance(r[0], str)]
    ...
    print_results(url, num_requests, concurrency, method, successful_requests, failed_requests,
...
def print_results(url, num_requests, concurrency, method, successful_requests, failed_requests,
                  avg_response_time, median_response_time, min_response_time, max_response_time):
    ...
    print(f"\n{Fore.GREEN}Successful Requests:{Style.RESET_ALL} {len(successful_requests)} "
          f"({len(successful_requests) / num_requests * 100:.2f}%)")
    print(f"{Fore.RED}Failed Requests:{Style.RESET_ALL} {len(failed_requests)} "
          f"({len(failed_requests) / num_requests * 100:.2f}%)")
    ...
    if failed_requests:
        print_error_summary(failed_requests)


def print_error_summary(failed_requests):
